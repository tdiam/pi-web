...
export function messageContent(msg: Pick<TranscriptEntryLike, "content" | "text">): string {
	...
	if (Array.isArray(content)) {
		return content
			.map((block: unknown) => {
				...
				if (typedBlock.type === "text" && typeof typedBlock.text === "string") return typedBlock.text;
				if (typedBlock.type === "toolResult" && typeof typedBlock.text === "string") return typedBlock.text;
				return "";
			})
			.filter(Boolean)
			.join("\n");
...
export function normalizeTranscript(messages: readonly TranscriptEntryLike[]): TranscriptEntryLike[] {
...
function appendToolResultToPreviousAssistant(
	...
	for (let index = normalized.length - 1; index >= 0; index--) {
		...
		const mergedContent = mergeToolResultIntoContent(candidate.content, toolResultMessage);
...
function mergeToolResultIntoContent(content: unknown, toolResultMessage: TranscriptEntryLike): unknown[] | null {
...
function findNextUnmatchedToolCallIndex(content: unknown[]): number {
	...
	for (let index = 0; index < content.length; index++) {
		...
		if (typedBlock.type === "toolResult" && unmatchedToolCallIndexes.length > 0) {
