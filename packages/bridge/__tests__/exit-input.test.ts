...
describe("isBridgeExitInput", () => {
	...
	it("recognizes selectCancel bindings from Pi keybindings", () => {
		const keybindings = {
			matches: vi.fn((input: string, action: string) => input === "kitty-ctrl-c" && action === "selectCancel"),
		};
	...
	it("recognizes copy bindings as a fallback", () => {
		const keybindings = {
			matches: vi.fn((input: string, action: string) => input === "kitty-ctrl-c" && action === "copy"),
		};
