...
export function createTerminalLogView(
	...
	const formatFooter = (): string[] => {
		return ["", "─".repeat(62), "", "Press Ctrl+C to stop the bridge and return to Pi"];
	};
...
export function createTerminalLogViewWithLogging(
	...
	return Object.assign(view, {
		handleLog(_message: string, _type: "info" | "client" | "error" | "shutdown" = "info") {
		...
		render(): string[] {
			...
			if (logs.length === 0) {
				...
				for (const entry of logs.slice(-15)) {
					const prefix = entry.type === "client"
						? "[C]"
						: entry.type === "error"
							? "[E]"
							: entry.type === "shutdown"
