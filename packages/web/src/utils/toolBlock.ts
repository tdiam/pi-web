...
export function buildToolCardModel(block: ToolContentBlock): ToolCardModel {
	...
	const meta = formatToolMeta(block.toolName, args, block.resultText, block.resultDetails, block.toolStatus, diffStats);
	const preview = formatToolPreview(block.toolName, args, block.resultText, block.resultDetails, block.toolStatus);
...
function buildToolDetails(block: ToolContentBlock, args: ToolArgsRecord | undefined): ToolCardSection[] {
	...
	if (block.toolName === "write" && writeContent && block.toolStatus !== "error") {
...
function formatToolTitle(toolName: string, args: ToolArgsRecord | undefined): string {
	switch (toolName) {
		case "read": {
			...
			return endLine !== undefined ? `${path}:${startLine}-${endLine}` : `${path}:${startLine}`;
		}
		case "bash": {
...
function formatToolMeta(
	...
	switch (toolName) {
		...
		case "edit": {
			...
			return edits !== undefined ? `${edits} replacement${edits === 1 ? "" : "s"}` : undefined;
		}
		case "write": {
...
function previewText(text: string | undefined, maxLines: number = 8): string | undefined {
...
function tailPreviewText(text: string | undefined, maxLines: number = 6): string | undefined {
	if (!text) return undefined;
	const normalized = text.replace(/\r/g, "").trim();
	if (!normalized) return undefined;
	const lines = normalized.split("\n");
...
function stringValue(args: ToolArgsRecord | undefined, key: string): string | undefined {
	const value = args?.[key];
	return typeof value === "string" ? value : undefined;
}

function numberValue(args: ToolArgsRecord | undefined, key: string): number | undefined {
	const value = args?.[key];
	return typeof value === "number" ? value : undefined;
}

function bashExitCode(resultText: string | undefined, status: ToolBlockStatus): number | undefined {
...
function diffStatsFromDiff(diffText: string | undefined): { added: number; removed: number } | undefined {
...
function buildDiffStats(
	...
	return {
		...stats,
		suffix: edits !== undefined ? `${edits} replacement${edits === 1 ? "" : "s"}` : undefined,
	};
}
