...
const createReadHighlighter = createBundledHighlighter<SupportedLanguage, SupportedTheme>({
...
export async function highlightCodeHtml(code: string, path?: string, themeMode: ThemeMode = "dark"): Promise<string> {
...
export function detectLanguageFromPath(path?: string): SupportedLanguage | "text" {
	if (!path) return "text";
