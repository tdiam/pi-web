...
describe("buildToolCardModel", () => {
	...
	it("uses file content previews for successful writes", () => {
		...
		expect(model.details[0]).toEqual({ label: "Content", text: "# Title\n\nBody copy" });
	});
	...
	it("shows bash exit code and only previews the last lines", () => {
		const block: ToolContentBlock = {
			...
			resultText: ["line 1", "line 2", "line 3", "line 4", "line 5", "line 6", "line 7", "Command exited with code 2"].join("\n"),
			toolStatus: "error",
		};
		...
		expect(model.preview).toBe(["... 2 earlier lines", "line 3", "line 4", "line 5", "line 6", "line 7", "Command exited with code 2"].join("\n"));
	});
	...
	it("renders real edit diffs directly in the preview", () => {
		...
		expect(model.diffStats).toEqual({ added: 2, removed: 1, suffix: "2 replacements" });
