...
const selectedKey = computed(() => (props.selectedModel ? getModelKey(props.selectedModel) : ""));
const filteredModels = computed(() => filterModels(props.models, searchText.value));
const triggerTitle = computed(() => {
	if (!props.selectedModel) return hasModels.value ? "Select Pi model" : "No Pi models available";
...
function scrollToHighlighted() {
	nextTick(() => {
		const element = listRef.value?.children[highlightedIndex.value] as HTMLElement | undefined;
	...
	switch (event.key) {
		...
		case "ArrowUp":
			...
			highlightedIndex.value =
				(highlightedIndex.value - 1 + filteredModels.value.length) % filteredModels.value.length;
			scrollToHighlighted();
			break;
		case "Enter": {
			event.preventDefault();
				...
				>
					...
					>
						<div class="model-option-copy">
							...
							<span class="model-option-meta">{{ model.provider }}/{{ model.id }}</span>
						</div>
						<Check
...
.model-menu {
	...
	border-radius: 16px;
	background:
		linear-gradient(180deg, color-mix(in srgb, var(--panel) 97%, transparent), var(--bg-elevated));
