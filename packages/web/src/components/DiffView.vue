...
const lines = computed(() => {
	return props.diff.replace(/\r/g, "").split("\n").map((line) => ({
		text: line,
		kind: classifyLine(line),
	}));
});

function classifyLine(line: string): "header" | "hunk" | "added" | "removed" | "context" {
...
<template>
	<div class="diff-view" role="presentation">
		<div v-for="(line, index) in lines" :key="`${index}:${line.text}`" class="diff-line" :data-kind="line.kind">
			<pre>{{ line.text }}</pre>
		</div>
	</div>
</template>
